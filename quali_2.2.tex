...
\chapter{Teoria de Otimização}
 	...
 	Variáveis:
 	I 	é o número de pares no sistema.
 	v	é o único vídeo que será distribuído, v=1.
 	H 	é o número de segmentos do filme. 
 	X 	é o tempo de playback do filme V.
 	ui 	é a capacidade de upload do par i
 	us	é a capacidade de upload do servidor
 	d	é a capacidade de download do par i
 	t	é tempo de latência antes da visualização do objeto.
 	svh 	é o h-ézimo segmento do filme v,todo segmento tem o mesmo tamanho e duração.
 	T 	é o tempo de duração do segmento de vídeo.
 	L 	é o tamanho do vídeo
 	l 	é o tamanho do segmento de vídeo
 	C 	é a capacidade do par i
 	W 	tamanho da janela de tempo para download de S segmentos
  	b	é a taxa de visualização
  	J	é o número de versões
  	S	é um swarm ( conjunto de pares ).

Neste modelo assumimos que uma rede P2P com N pares apoia um servidor na distribuição de segmentos de vídeos em um sistema CDN-P2P, que distribui vídeos sob demanda por meio de streaming adaptativo, de modo que, é necessário decidir quais segmentos e de quais versões serão baixados, tal decisão deve considerar a melhora na percepção da qualidade do vídeo pelo usuário e também a diminuição da carga sobre o servidor, cada par tem espaço de armazenamento c e diferentes disponibilidades de banda de upload u, que assumimos estarem ordenadas de forma decrescente u1 > u2 > - - - > uN,(Xu:2013) e download d que variam com o tempo, para guardar e compartilhar conteúdo com a comunidade.
.
...
As fórmulas \ref{eq:equacao-exemplo1} e são exemplos de como apresentar fórmulas e equações destacadas do parágrafo normal do texto. Veja mais detalhes no anexo \ref{anexoA} deste documento.
