...
\section{Distribuição de Conteúdo na Internet.}

 (Zhao, Mingchen et al. 2013) abordam as tradicionais arquiteturas adotadas para distribuição de conteúdo; Redes de distribuição de conteúdo (CDNs) em que os clientes baixam conteúdo de servidores dedicados e gerenciados centralmente e da arquitetura híbrida ou “peer assisted CDNs”.
O artigo exibe os potenciais riscos e benefícios das  Peer assisted CDNs e utiliza como instância de estudo o sistema NetSession da Akamai para examinar na prática os impactos desses riscos e benefícios.
O NetSession é operado comercialmente desde 2010, possui atualmente mais de 25 milhões de usuários em 239 países. Nos estudos apresentados para esta instância, o NetSession pode descarregar 70\%-80\% do tráfego para os pares sem uma proporcional perda de performance ou segurança, o artigo conclui que os riscos podem ser bem gerenciados e sugere que sistemas híbridos são opções atrativas para futuras CDNs.

(Araujo,Júlio et al, 2011) sugerem a introdução de caches de conteúdo em cada roteador do backbone, com o objetivo de reduzir o congestionamento no centro da rede e melhorar a eficiência na distribuição de conteúdo; a instância estudada utiliza caches em CDNs, tem o objetivo de encontrar rotas eficientes e armazenar nas caches os conteúdos mais populares, levando em consideração a abrangência de parâmetros como: tamanho da cache, popularidade do conteúdo, intensidade da demanda. 

(Osman, Niemah et al, 2011) Neste trabalho aponta-se a otimização de tamanhos de cache para armazenar o mais popular conteúdo próximo do usuário para serviços de VoD que possuem alta demanda de trafego, reduzindo a carga sobre o centro da rede. Apresentam um modelo em programação linear inteira mista  para otimizar o tamanho da cache para cada nó da rede em diferentes horários do dia, ajustando ao volume do conteúdo.

(Saha, Sumanta  et al,2011) apresentam uma solução integrando redes P2P e tradicionais CDN, chamada GroupDL que consiste em um grupo de amigos adquirir uma copia multi-licenciada de um conteúdo original do servidor e então colaborar para distribuir entre si.
Mostra através de simulação que somente 1/n-ésimo da largura de banda original é requerida.

\subsection{Arquitetura Híbrida CDN-P2P}

(Mandal, Uttam et al. 2014) abordam sobre a popularidade crescente de serviços baseados em conteúdo, em que redes de distribuição de conteúdo ( CDNs) armazenam e distribuem esses conteúdos, utilizando múltiplos locais de armazenamento para balanceamento de carga, disponibilidade e acesso mais rápido, porém, ressalta que escalabilidade e custos de CDNs estão se tornando grandes desafios devido ao crescimento desse tipo de serviço.
O artigo aponta para a arquitetura peer-to-peer (P2P) como combate inicial a esses desafios, combinando as arquiteturas CDN e P2P em um sistema híbrido; teórica e praticamente tem se mostrado como um sistema factível, menos custoso e escalável.
O estudo analisado demonstra que é possível com o uso de redes P2P em apoio a CDN na distribuição de conteúdo, uma redução de carga de 30\%-40\% sobre o servidor da CDN na distribuição de conteúdo moderadamente popular.

(Maki, N. et al., 2013)Neste artigo propõe-se um esquema de engenharia de trafego para peer assisted CDN, que são CDNs auxiliadas por pares de uma rede P2P, que fazem cache de conteúdo e são apontados como fontes destes para clientes próximos que requisitam estes mesmos conteúdos, que por sua vez pode ser armazenado por inteiro ou em fragmentos.
É proposto um mecanismo de incentivo orientado a conteúdo para motivar clientes da rede P2P a fazer cache de conteúdo; o mecanismo de incentivo consiste em combinar conteúdos a preço de apenas um conteúdo, em troca,  o cliente chamado altruísta faz cache desses conteúdos e pode servi-los individualmente a outros pares.
Propõe-se um formulação para o problema da otimização da seleção dos arquivos que serão combinados.
Estabelecem limiares e através de simulações avaliam como relevante a diminuição do trafego de dados para o centro da rede e consequentemente dos custos destes. Mas, alertam para o problema do aumento do trafego em determinados períodos em que os arquivos de conteúdos combinados são baixados para os pares altruístas.
...
Neste capítulo a replicação controlada de conteúdo é caracterizada como um problema de programação linear Inteira. A replicação ocorre em uma rede CDN-P2P dedicada a distribuição de vídeos com taxas de consumo adaptável as condições de rede. Os $N$ pares que participam do swarming $P$, formando em torno do distribuição do vídeo $v$, aceitam compartilhar segmentos de $v$ replicados de forma pró ativa.

O vídeo $v$ é versionado com diferentes taxas de bits $(r_1,r_2,\ldots,r_J)$, i.e. qualidades, e cada versão é segmentada e alinhada no tempo, permitindo que os pares acessem, a qualquer momento, segmentos de qualidade distintas. As informações de todas as versões de $v$ encontra-se em uma playlist $Z$, repassada ao par $i$ quando este se junta ao swarming. Usando as informações em $Z$, um algoritmo de seleção deve escolher, em um determinado instante $t$, qual é a qualidade do próximo segmento a ser baixado. Essa decisão deve considerar as seguintes variáveis:
\begin{itemize}
\item os recursos de transmissão disponível;
\item a continuidade da sessão de acesso;
\item a qualidade do segmento acessado;
\end{itemize} 

O recurso de transmissão de um par $i$ é limitado e definido por suas taxas de upload $u_i$ e download $d_i$. Em outras palavras, ao atuar como servidor no swarming, o par $i$ estará limitado no atendimento pela suas taxa de upload. E ao atuar como cliente, ele estará limitado por sua capacidade acesso a rede.

A continuidade da sessão de acesso é impactada pelo conjunto de segmentos que devem estar disponíveis para a reprodução em um futuro breve. 

bem como a sua capacidade de armazenamento $S_i$.  A Figura \ref{fig:StreamingAdaptativo} mostra os elementos e a dinâmica descrita anteriormente.

\begin{figure}[H]% H manda colocar exatamente nessa posição no texto (relativa aos parágrafos anterior e posterior)
	\centering
	\includegraphics[height=5cm,width=0.8\textwidth]{figuras/Jversoes.png}
