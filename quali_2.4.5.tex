...
Neste capítulo apresenta-se o problema da replicação de conteúdo em uma rede CDN-P2P dedicada a distribuição de vídeos com qualidades adaptáveis as condições de rede. Assume-se que a rede P2P, com $N$ pares, apoia a distribuição de um vídeo feita pelos servidores da CDN. A Rede P2P implementa o protocolo bittorrent e por isso os pares envolvidos na distribuição de um vídeo $v$ formam um swarming $P$ em torno de $v$.

Os pares que formam o swarming $P$ acessam o vídeo considerando que o fluxo de bits pode ser adaptado as condições da rede. Em outras palavras, o vídeo distribuído nesse sistema foi versionado com diferentes taxas de bits $(r_1,r_2,\ldots,r_J)$, i.e. qualidades, e cada versão foi fragmentada e alinhada no tempo, permitindo o acesso, a qualquer momento, das qualidades disponíveis. O acesso as diferentes qualidades é feita a partir de uma playlist $Z$, e um algoritmo de seleção deve escolher, em um determinado instante $t$, qual é a qualidade do próximo segmento a ser baixado. Essa decisão deve considerar os recursos de transmissão disponível em uma janela de tempo $W_i$ e que o vídeo precisa ser servido com a melhor qualidade possível. O recurso de transmissão de um par $i$ é limitado e definido por suas taxas de upload $u_i$ e download $d_i$, bem como a sua capacidade de armazenamento $S_i$.  A Figura \ref{fig:StreamingAdaptativo} mostra os elementos e a dinâmica descrita anteriormente.

\begin{figure}[H]% H manda colocar exatamente nessa posição no texto (relativa aos parágrafos anterior e posterior)
	\centering
i	\includegraphics[height=5cm,width=0.8\textwidth]{figuras/Jversoes.png}
 	 \caption{Streaming adaptativo em uma rede P2P}
	\label{fig:StreamingAdaptativo}
  %\source{Autor, 2016}
\end{figure}

Neste cenário, considera-se que os pares no swarming estão desejosos em compartilhar o conteúdo injetado no par por uma política de replicação pró-ativa implementada no sistema. O problema modelado a seguir considera 

o problema de otimização da disponibilidade de segmentos de maior qualidade, em termos de bits recebidos na rede P2P. Em um Swarming $P$ de $N$ pares e uma playlist $Z$ com $J$ versões de vídeo com $r$ taxas decrescentes $r_j=(r_1,r_2,\ldots,r_J)$ de um vídeo $v$ a ser distribuido na comunidade, é necessário decidir quantas cópias do segmento $h_j$ serão criadas, para que se mantenha ou diminua a carga sobre o servidor e considere a melhor percepção da qualidade de vídeo para o usuário dentro do swarm.
...
Na distribuição de vídeo em uma comunidade SEM streaming adaptativo o número de cópias de qualquer objeto é um inteiro a qualquer momento. Seja $S_i$ a capacidade de armazenamento do par $i$, dada em bytes, então a capacidade total de armazenamento da comunidade $S$ é dada por:
...
Seja $x_{iv}$ uma variável decisão que assume o valor 1 se o par $i$ contém uma cópia do vídeo $v$ e 0(zero) caso contrário.
